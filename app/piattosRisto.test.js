...
/**
 * Per chi tocca fare la post per aggiungere piatto, inserire prima di qua in modo che poi posso eliminare il piatto appena inserito. 
 * 
 * 
 */
 //test delete piatto di piatto esistente e token corretto
  test('DELETE /api/v1/piattosRisto/eliminaPiatto existing dish should return 204 --> deleted dish', async () => {
    ...
    //expect.assertions(1);
    const response= await request(app)
    .delete('/api/v1/piattosRisto/eliminaPiatto/62971f36f6d21ae1a617eef1')
    
    .set('x-access-token', token)
    .set('Content-Type', 'application/json')
    //'Content-Type': 'application/json' , 'x-access-token': loggedUser.token
